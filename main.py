...
The `query` subcommand searches for close approaches that match given criteria:
    ...
    $ python3 main.py query --start-date 2020-01-01
        --end-date 2020-01-31 --max-distance 0.025
    $ python3 main.py query --start-date 2050-01-01
        --min-distance 0.2 --min-velocity 50
    $ python3 main.py query --date 2020-03-14
        --max-velocity 25 --min-diameter 0.5 --hazardous
    $ python3 main.py query --start-date 2000-01-01
        --max-diameter 0.1 --not-hazardous
    $ python3 main.py query --hazardous
        --max-distance 0.05 --min-velocity 30
...
without having to wait to reload the database each time.
However, it doesn't hot-reload.
...
def make_parser():
    ...
    inspect.add_argument('-v', '--verbose', action='store_true',
                         help="Additionally, print all known close approaches\
                              of this NEO.")
    ...
    filters.add_argument('--min-diameter', dest='diameter_min', type=float,
                         help="In kilometers. Only return close approaches of\
                              NEOs with "
                              "diameters as large or larger than the given\
                                  size.")
    filters.add_argument('--max-diameter', dest='diameter_max', type=float,
                         help="In kilometers. Only return close approaches of\
                              NEOs with "
                              "diameters as small or smaller\
                                  than the given size.")
    ...
    repl = subparsers.add_parser('interactive',
                                 description="Start an interactive command\
                                      session "
                                             "to repeatedly run `interact` and\
                                                  `query` commands.")
...
class NEOShell(cmd.Cmd):
    ...
    def do_query(self, arg):
        ...
        command line. For example, to query close approaches
        on January 1st, 2020:
